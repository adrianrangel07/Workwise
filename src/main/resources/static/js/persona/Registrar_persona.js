...
function sendVerificationCode() {
    ...
    if (!validateEmail(email)) {
        showVerificationStatus('Por favor ingresa un correo electrónico válido', 'error');
        return;
    }

    // Configurar estado de carga
    const btnText = document.querySelector('#send-code-btn .btn-text');
    const btnIcon = document.querySelector('#send-code-btn .btn-icon');
    ...
    fetch('/api/send-verification-email?email=' + encodeURIComponent(email), {
        ...
        .then(data => {
            ...
            // Mostrar éxito
            showVerificationStatus('Código enviado correctamente. Revisa tu correo.', 'success');
        ...
        .catch(error => {
            ...
            showVerificationStatus('Error al enviar el código: ' + error.message, 'error');
...
document.getElementById('verify-code-btn').addEventListener('click', function () {
    ...
    if (!code || code.length !== 6) {
        showVerificationStatus('Por favor ingresa un código válido de 6 dígitos', 'error');
        return;
    }
    ...
    // Enviar solicitud al servidor
    fetch('/api/verify-code?email=' + encodeURIComponent(currentEmail) + '&code=' + code, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        }
...
document.getElementById('verification-code').addEventListener('input', function () {
    ...
    // Habilitar botón solo si el código tiene 6 dígitos
